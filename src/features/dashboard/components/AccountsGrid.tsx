...
import { getFinancialAccounts } from "../action/queries";
import { Skeleton } from "@/components/ui/skeleton";

export async function AccountsGrid({
  userId,
  locale,
}: {
  userId: string;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {[1, 2, 3, 4].map((i) => (
        <div
          key={i}
          className="bg-card border border-border/50 rounded-[32px] p-6 shadow-sm space-y-6"
        >
